...
const enemyForYou = [
  ...
  "with CAFFEINE",
  "by DELETING & STARTING OVER",
  "with a CONSOLE.LOG",
  "with a DEV TOOLS BREAKPOINT",
  "with the SOURCES TAB",
  "by sending a SLACK CHANNEL MESSAGE",
  "by Zooming with a DEVELOPER FRIEND",
];
...
if (options[readyToGo] === "Yes" || options[readyToGo] === 1) {
  ...
  while (!gameOver) {
    if (
      playerTotals.healthPoints > 0 &&
      playerTotals.healthPoints < 100 &&
      inventoryOfErrorsArr.length < 3
    ) {
      //game continues if still have HP left; if YES, then WALK
      walk();
    } else if (playerTotals.healthPoints <= 0) {
      //else, if HP is zero or negative, then game ends
      byeByePlayer();
    } else if (
      playerTotals.healthPoints >= 100 ||
      inventoryOfErrorsArr.length >= 3
    ) {
      //player(a.k.a. the bug) wins the game
...
function welcomeGreeting() {
  ...
  console.log(
    `~~Your mission is to SURVIVE so that you can keep messing with the program...and the programmer!!!)~~`
  );
  console.log(`\nYou'll start with a total of 75 health points.`);
  console.log(
    `Each time you attack the developer OR the developer attacks you,`
  );
  console.log(
    `you will feel a little less buggy and your health points will DECREASE.`
  );
  ...
  console.log(
    `so you can throw it at the next crafty software developer who comes your way!`
  );
  console.log(
...
function walk() {
  const walking = readline.question(
    `\nKEEP IT BUGGY!  Please enter W (to WALK through the code) or X (to end game) `,
    { trueValue: ["w", "W"], falseValue: ["x", "X"] }
  ); //ADD P for tallies to this menu??
  if (walking === true) {
    //when player selects "W" or "w"
    console.log(`\nHere we go!`);
    maybeEnemy();
  } else if (
    walking === true &&
    (aOrR[makeDecision] === "Attack" || aOrR[makeDecision] === 0)
  ) {
    enemyAttacksBack();
    viewPlayerTotals();
  } else if (
    walking === true &&
    (aOrR[makeDecision] === "Run" || aOrR[makeDecision] === 1)
  ) {
    enemyAttacksBack();
    viewPlayerTotals();
  } else if (walking === false) {
    //when player selects "X" or "x"
    gameOver = true;
    console.log(
...
function maybeEnemy() {
  ...
  if (enemy === 0 || enemy === 2) {
    //if 0 or 2 is randomly returned, then you are attacked.
    console.log(`\nUH OH!  You are under ATTACK from the evil programmer!`);
    uhOhEnemy();
    attackOrRun();
  } else {
    //if 1 or 2 is generated, then the bug keeps walking through the program.
    console.log(
...
function attackOrRun() {
  ...
  const makeDecision = readline.keyInSelect(
    aOrR,
    `OK, ${characterName}, Are you going to ATTACK or RUN?`
  );
  if (aOrR[makeDecision] === "Attack" || aOrR[makeDecision] === 0) {
    //decided to ATTACK
    console.log(
    ...
    //enemyAttacksBack();
    viewPlayerTotals();
  } else if (aOrR[makeDecision] === "Run" || aOrR[makeDecision] === 1) {
    //user decided to RUN
    ...
    if (runFastEnough === 0) {
      //if returns 0: then ESCAPED BY RUNNING
      console.log(
        `Good thing you're fast, ${characterName}!!  You are safe FOR NOW.`
      );
      //enemyAttacksBack();
      viewPlayerTotals();
    } else if (runFastEnough === 1) {
      //if returns 1: then you ran, BUT NOT FAST ENOUGH & bug (player) is exterminated
      console.log(
...
function enemyAttacksBack() {
  ...
  if (
    ...
    //If the player kills the enemy:give the Player some HP and a special item that is stored in the inventory.
...
function viewPlayerTotals() {
  //check player's game tallies
  const checkTallies = readline.keyInSelect(
    pOrC,
    `\nIf you would like to check your game totals, select "Print".  Otherwise, select "Continue" `
  ); //Option To Print Tallies OR Continue
  if (
    (pOrC[checkTallies] === 0 || pOrC[checkTallies] === "Print") &&
    playerTotals.healthPoints > 0 &&
    playerTotals.healthPoints < 100 &&
    inventoryOfErrorsArr.length < 3
  ) {
    //when player selects "Print"; also, checks for positive HP total
    ...
    //when player selects "Print" & no HP left or negative HP
    ...
    (pOrC[checkTallies] === 1 || pOrC[checkTallies] === "Continue") &&
    playerTotals.healthPoints > 0 &&
    playerTotals.healthPoints < 100 &&
    inventoryOfErrorsArr.length < 3
  ) {
    //when player selects "Continue"; checks for positive HP total
    enemyAttacksBack();
    ...
    //when player selects "Continue" & no HP left or negative HP
    ...
    (pOrC[checkTallies] === 1 || pOrC[checkTallies] === "Continue") &&
    (inventoryOfErrorsArr.length >= 3 || playerTotals.healthPoints >= 100)
  ) {
    console.log(`\nCONGRATULATIONS, ${characterName}!  You've WON!`);
    console.log(`\nYou have worn out all of the developers...`);
    console.log(`and been crowned the BUGGIEST BUG in town!`);
...
//will's readline-sync version
// import readlineSync from "readline-sync";

// const readlineSync = require("readline-sync");

// const name = readlineSync.question(
//   "Hey there young Pokemon Trainer! What is your name?\n"
// );
...
// const treasure = [
//   "1 entire Bitcoin",
//   "Redbull and a Chikfila Sandwich",
//   "Tesla",
// ];
// var prize = [];
// let userHealth = 30;
...
//     } else if (key >= 0.3) {
//       console.log(
//         "A wild " + wildPokemon + " has appeared!\n What would you like to do?"
//       );
...
//           'Enter "r" to run, or enter "a" to attack! '
//         );
...
//                 "You Cant escape! " +
//                   wildPokemon +
//                   " attacked you, causing a damage total of: " +
//                   wildPokemonPower +
//                   " Hit Points."
//               );
//             } else {
...
//             console.log(
//               "Enemy Pokemon just attacked you with " +
//                 wildPokemonPower +
//                 " Hit Points."
//             );
...
//               console.log(
//                 wildPokemon +
//                   " has defeated your Pokemon. You blacked out! You will respawn at the nearest Pokemon Center."
//               );
//             }
//         }
//       }
