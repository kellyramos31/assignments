  ...
  Flashcard.findOneAndDelete(
    ...
    (err, deletedFlashcard) => {
      ...
      return res
        .status(200)
        .send(
          `Successfully deleted comment: ${deletedFlashcard.firstName}${deletedFlashcard.lastName}`
        );
    }
  );
});
...
learnGameRouter.delete("/play/:questionId", (req, res, next) => {
  Question.findOneAndDelete(
    ...
    (err, deletedQuestion) => {
      ...
      return res
        .status(200)
        .send(`Successfully deleted question: ${deletedFlashcard.answer}`);
    }
  );
});
...
learnGameRouter.get("/learn/search/categorySTEM", (req, res, next) => {
  ...
  Flashcard.find(
    { categorySTEM: { $regex: pattern, $options: "i" } },
    (err, categories) => {
      if (err) {
        res.status(500);
        return next(err);
...
learnGameRouter.get("/learn/search", (req, res, next) => {
  const { searchTerm } = req.query;
  //changed logic to work with Atlas -- NOTE:  currently does exact matches only (not partial matches)
  Flashcard.aggregate(
    [
      {
        $search: {
